...
class HexValidator {
  ...
  play(cell:number|null){ if(!this.legal(cell)) throw new Error('Illegal move'); if(cell===null){ this.board=this.board.map(c=>c===1?2:c===2?1:0) as (0|1|2)[]; this.turn++; return; } const color=this.turn===1?1:2; this.board[cell]=color; const dsu=color===1?this.dsu1:this.dsu2; for(const nb of this.getNeighbors(cell)){ if(this.board[nb]===color) dsu.union(cell, nb); } if(color===1){ const col=cell%this.n; if(col===0) dsu.union(cell,this.board.length); if(col===this.n-1) dsu.union(cell,this.board.length+1); } else { const row=Math.floor(cell/this.n); if(row===0) dsu.union(cell,this.board.length); if(row===this.n-1) dsu.union(cell,this.board.length+1); } this.turn++; }
