...
export default function Auth() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'discord') => {
    ...
    try {
      const signInFn = provider === 'google' ? signInWithGoogle : signInWithDiscord;
      const { error } = await signInFn();
      if (error) {
        toast({
          title: `${provider === 'google' ? 'Google' : 'Discord'} login failed`,
              ...
              <>
                ...
                <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-red-500/10 border border-amber-500/20">
                  ...
                  <p className="text-center text-sm text-muted-foreground mb-3">
                    Our developers are "working hard" on these buttons
                  </p>
                  ...
                  <p className="text-center text-xs text-muted-foreground/60 mt-2 italic">
                    (any day now... probably... maybe)
                  </p>
                </div>
